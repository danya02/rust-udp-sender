...
use common::{networking::send_message, messages::JoinReason};
...
pub async fn discover_server(channel: &mut common::MessageReceiver, my_name: &str, server_name: Option<&str>) -> Option<SocketAddr> {
    ...
    loop {
        ...
        if server_name.is_none() || their_name == server_name.unwrap() {
            debug!("Received packet from {} ({}): {:?}", their_addr, their_name, message);
            match message {
                ...
                common::messages::Message::JoinResponse(reason) => {
                    if expecting_join_ok_from.is_none() {continue;}
                    ...
                    if their_addr == expecting_ip_addr {
                        ...
                        if reason == JoinReason::Accepted {
                            ...
                            error!("Server rejected our join request with this reason: {:?}", reason);
